...
contract TestK1ValidatorFactory_Deployments is NexusTest_Base {
    ...
    function setUp() public {
        ...
        bootstrapper = new NexusBootstrap(address(DEFAULT_VALIDATOR_MODULE), address(0), abi.encodePacked(address(0xeEeEeEeE)), "");
        validatorFactory =
    ...
    function test_ConstructorInitializesFactory() public {
        ...
        NexusBootstrap bootstrapperInstance = new NexusBootstrap(address(DEFAULT_VALIDATOR_MODULE), address(0), abi.encodePacked(address(0xeEeEeEeE)), "");
        ...
        bytes memory _initData = abi.encode(
            ...
            abi.encodeCall(
                ...
                (
                    ...
                    "",
                    RegistryConfig({ registry: validatorFactory.REGISTRY(), attesters: attesters, threshold: threshold })
                )
            )
        );
